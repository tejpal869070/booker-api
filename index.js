...
async function addWalletStatement(
  id,
  email,
  amount,
  game_name,
  ...
  try {
    const user_game_wallet_balance = await queryAsync(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const checkAddressQuery =
        "SELECT * FROM usdt_address where usdt_address = ?";
      const checkAddressResult = await queryAsync(checkAddressQuery, [
        deposit_to,
      ]);
      if (checkAddressResult.length === 0) {
        return res
          .status(404)
          .send({ message: "You are not depositing on our address." });
      }

      const duplicateDeposit =
        "SELECT transection_hash FROM deposit WHERE transection_hash = ?";
      const duplicateDepositResult = await queryAsync(duplicateDeposit, [
        transection_hash,
      ]);

      const alreadyDeposit =
        "SELECT * FROM deposit where user_id = ? AND status = 'P'";
      const alreadyDepositResult = await queryAsync(alreadyDeposit, [
        req.user.user_id,
      ]);
      if (alreadyDepositResult.length > 0) {
        return res
          .status(409)
          .send({ message: "An Deposit request is already in process !" });
      }
      ...
      if (duplicateDepositResult.length > 0) {
        ...
        res
          .status(200)
          .send({ message: "Deposit Request Added Successfully!" });
      }
    } catch (error) {
      fs.unlinkSync(path.join(__dirname, "assets", image.filename));
      res.status(500).send({ message: "Internal Server Error" });
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const findWithdrawalQuery =
        "SELECT status FROM withdrawal WHERE email = ? AND status = ?";
      const findWithdrawalResult = await queryAsync(findWithdrawalQuery, [
        email,
        "P",
      ]);
      if (findWithdrawalResult.length > 0) {
        ...
        const findUserBalanceQuery =
          "SELECT main_wallet FROM wallet WHERE user_id = ?";
        ...
        if (findUserBalanceResult.length > 0) {
          if (findUserBalanceResult[0].main_wallet < amount) {
            ...
            const createWithdrawalResult = await queryAsync(
              createWithdrawalQuery,
              { email, withdrawal_address, amount }
            );
            if (createWithdrawalResult.affectedRows > 0) {
              // deduct wallet balance---------------------------
              const deductWalletBalanceQuery =
                "UPDATE wallet SET main_wallet = main_wallet - ? WHERE user_id = ?";
...
app.post("/deduct-game-wallet", verifyToken, async (req, res) => {
  ...
  if (!email || isNaN(amount) || amount <= 0 || !type || !game_type) {
    res
      ...
      .send({
        message:
          "All fields are required and amount should always be a positive number!",
      });
  }
    ...
    if (type === "deduct") {
      ...
      if (findUserBalanceResult.length > 0) {
        if (findUserBalanceResult[0].game_wallet < amount) {
          ...
          // deduct game wallet---------------------------
          const deductGameWalletQuery =
            "UPDATE wallet SET game_wallet = game_wallet - ? WHERE email = ?";
          const deductGameWalletResult = await queryAsync(
            deductGameWalletQuery,
            [amount, email]
          );
      ...
      // add game wallet---------------------------
      const addGameWalletQuery =
        "UPDATE wallet SET game_wallet = game_wallet + ? WHERE email = ?";
      const addGameWalletResult = await queryAsync(addGameWalletQuery, [
        amount,
        email,
      ]);
